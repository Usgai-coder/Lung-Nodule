...
医学影像（如胸部 CT）的深度学习预处理往往面临着严苛的临床一致性与工程防泄露要求。本项目基于国际权威的 LIDC-IDRI 及其衍生基准 LUNA16，打造了一套从原始异构医疗数据（DICOM/MHD + XML）到深度学习标准张量的端到端全自动预处理流水线。

项目在工程与算法层面，核心突破了以下四大行业痛点：

各向同性重采样与肺实质精准分割 (Isotropic Resampling & Lung Segmentation)：

针对 CT 扫描 Z 轴层厚不均引发的 3D 卷积形变，强制执行 1 x 1 x 1 mm 的等方性三维线性插值。

采用纯 SimpleITK 构建三维形态学闭运算管道，精准剥离胸壁与扫描床。摒弃常规掩膜的“乘零”错误，引入条件映射将肺外背景严格还原为代表空气的 -1000 HU，确保模型物理语义的绝对正确。

阅片者间变异性消除 (Inter-reader Variability)：

引入 3D 物理空间欧氏距离聚类算法，强制合并多位医生在空间距离 <= 5.0 mm 内圈定的同源病灶，生成零争议的“黄金结节”大账本。

三维局部裁切与归一化 (3D ROI & Windowing)：

在三维绝对物理坐标系下精准定位，裁切 32 x 32 x 32 体素的局部立方体，并完成肺窗（[-1000, 400]）的绝对线性归一化。

严格患者级防泄露切分 (Strict Patient-Level Split)：
...
│       ├── all_roi_cubes/          # 模块二：LIDC 提取的 32x32 结节局部 ROI (.npy)
│       ├── master_data_exploration.csv    # 临床全维度大账本
│       ├── cleaned_nodules_labels.csv     # 高质量黄金结节清单
│       ├── final_dataset_manifest.csv     # 挂载双/多分类标签的资产总账
│       └── final_split_train_manifest.csv # 终极 7:1:2 防泄露切分索引表
│
├── src/                            # 核心闭环源码
│   ├── preprocess.py               # [基础管道] LUNA16 等方性重采样与三维形态学肺实质分割
│   ├── explore_data.py             # [特征挖掘] LIDC 空间物理聚类与标注一致性清洗
│   ├── extract_roi.py              # [张量构建] 物理坐标对齐、3D ROI 裁切与 HU 截断
│   └── split_dataset.py            # [数据切分] 最大风险锁定、患者级防泄露 7:1:2 发牌
│
└── README.md                       # 本说明文档
...
🛠️ 模块一：医学图像全肺标准化预处理验证 (基于 LUNA16)
...
🛠️ 模块二：多维特征清洗与 3D ROI 构建 (基于 LIDC-IDRI)
...
以患者 ID 为底层单元执行洗牌，完美对齐 72% : 10% : 18% 黄金交叉，输出 3D-CNN DataLoader 的标准索引。
